...
def create_doctor_table():
    cursor.execute('CREATE TABLE IF NOT EXISTS doctordb(doctorname TEXT,specialization TEXT,mobileno NUMBER)')


def add_doctor_data(doctorname, specialization, mobileno):
    cursor.execute('INSERT INTO doctordb(doctorname,specialization,mobileno) VALUES (?,?,?)', (doctorname, specialization, mobileno))
    conn.commit()
...
def admin_page():
    ...
    if goto == "Add Doctor":
        ...
        if st.button("Add Doctor"):
            create_doctor_table()
            add_doctor_data(doctorname, specialization, mobileno)
